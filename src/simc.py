...
def simPrep(confDict):
	...
	# Lat/Long, XYZ, echomap systems. [llsys, xyzsys, ecosys].
	systems(confDict, simParams)
...
def runSim(confDict, simParams, data):
	...
	Args:
		...
		data(dict): Arrays to hold clutter simulation data and
					transform nav file to XYZ
	"""
	print('\033[92m\033[1mSimulating {} traces: '.format(
		len(simParams['navxyz'])))
	...
	for i in range(len(simParams['navxyz'])):
		...
		# Right side of spacecraft.
		py_sim(confDict, data, simParams['navxyz'][i], 'r')
		# Left side of spacecraft.
		py_sim(confDict, data, simParams['navxyz'][i], 'l')
...
def saveProducts(confDict, simParams, data):
	...
	if confDict['outputs']['combined_adjusted'] == 'True':
		data['cgram'] = data['cgram'] * \
			(255.0/(data['cgram'].max()-.1*data['cgram'].max()))
	...
	if confDict['outputs']['echomap'] == 'True':
		...
		echomap(confDict, simParams, data, [
				escale, 1, simParams['num_ct']], False)

	# Produce echomap_adjusted.png product.
	if confDict['outputs']['echomap_adjusted'] == 'True':
		yDistortion = int(confDict['facet_params']
						  ['ct_step']) / simParams['nad_xyz'].avgdiff()
		emap[emap != 0] = emap[emap != 0] - \
			(emap[emap != 0].mean()-emap[emap != 0].std())
		...
		echomap(confDict, simParams, data, [
				nemap, yDistortion, yDimension//2], True)

	# Produce echomap_ref.tif product.
	if confDict['outputs']['echomap_ref'] == 'True':
		driver = gdal.GetDriverByName('GTiff')
		dataSet = driver.Create(confDict['paths']['out_path'] + 'echomap_ref.tif',
								simParams['exs'], simParams['eys'], 1, GDT_Float32)
		...
		dataSet.GetRasterBand(1).WriteArray(
			simParams['emap_ref'].astype('float32'))
