...
def configChecks(confDict):
    ...
    if at_dist < at_step:
        raise SystemExit("\033[91m\033[1mINVALID PARAM!\nALONG TRACK DIST < ALONG TRACK STEP")
    if ct_dist < ct_step:
...
def systems(confDict, simParams):
    ...
    if confDict['sim_params']['body'] == 'mars':
        llesys = '+proj=longlat +a=3396190 +b=3376200 +no_defs'  # IAU Mars 2000.
        xyzsys = '+proj=geocent +a=3396190 +b=3376200 +no_defs'  # IAU Mars 2000.
    ...
    if confDict['sim_params']['body'] == 'moon':
        llesys = '+proj=longlat +a=1737400 +b=1737400 +no_defs'  # IAU Moon 2000.
        xyzsys = '+proj=geocent +a=1737400 +b=1737400 +no_defs'  # IAU Moon 2000.
    if confDict['sim_params']['body'] == 'earth':
        llesys = '+proj=longlat +a=6378140 +b=6356750 +no_defs'  # IAU Earth 2000.
        xyzsys = '+proj=geocent +a=6378140 +b=6356750 +no_defs'  # IAU Earth 2000.
    ...
    print("\033[1mGrid:\033[0m " + confDict['facet_params']['at_dist'] + "m along track, " + confDict['facet_params']['ct_dist'] + "m cross track")
    print("\033[1mFacet:\033[0m " + confDict['facet_params']['at_step'] + "m along track, " + confDict['facet_params']['ct_step'] + "m cross track")
...
def miscSimPrep(confDict, simParams):
    ...
    simParams['navdat'] = eval(confDict['navigation']['navfunc'])(confDict['paths']['nav_path']) 
    simParams['navdat'].csys = confDict['navigation']['navsys'] 

    # Define number of along track and cross track points to make facets from.
    simParams['num_ct'] = int(confDict['facet_params']['ct_dist'])//int(confDict['facet_params']['ct_step'])
    simParams['num_at'] = int(confDict['facet_params']['at_dist'])//int(confDict['facet_params']['at_step'])  # Not used.

    # This is specific to making FPB sims with areoid referenced products.
    simParams['nad_loc'] = simParams['navdat'].toground(simParams['topo'],confDict['navigation']['navsys'])
    simParams['nad_xyz'] = simParams['nad_loc'].transform(simParams['sys'][1])  # Transform xyzsys.
...
def echomapPrep(confDict, simParams):
    ...
    if confDict['outputs']['echomap_ref'] == 'True':
        ecom = simParams['navdat'].transform(simParams['sys'][2])  # Transform ecosys.
        # Get grid limits and step size for output echomap_ref.
        maxdist = max(int(confDict['facet_params']['at_dist']), int(confDict['facet_params']['ct_dist']))
        ...
        epsize = max(int(confDict['facet_params']['at_step']), int(confDict['facet_params']['ct_step'])) * 12
...
def dataArrays(confDict, simParams):
    ...
    if confDict['outputs']['echomap'] == 'True':
        data['emap'] = np.zeros((2*simParams['num_ct'],len(simParams['navdat'])))
        data['bins'] = np.ones((2*simParams['num_ct'],len(simParams['navdat'])))*int(confDict['sim_params']['tracelen'])

    # Make empty arrays to hold the right and left side clutter simulations.
    data['rcgram'] = np.zeros([int(confDict['sim_params']['tracelen']),len(simParams['navdat'])])
    data['lcgram'] = np.zeros([int(confDict['sim_params']['tracelen']),len(simParams['navdat'])])
    data['rRed'] = np.zeros([int(confDict['sim_params']['tracelen']),len(simParams['navdat'])])
    data['lRed'] = np.zeros([int(confDict['sim_params']['tracelen']),len(simParams['navdat'])])
    data['tmap'] = [None]*len(simParams['navdat'])

    # Keeping track of the bins data is assigned to.
    data['fret_loc'] = Path(simParams['sys'][1],[Loc(0,0,0)]*len(simParams['navdat']))  # xyzsys.
    data['minbin'] = [None]*len(simParams['navdat'])
    
    # Transform navigation file to XYZ.
    simParams['grdlle'] = simParams['navdat'].toground(simParams['topo'],simParams['sys'][0])  # Not used.
    simParams['navxyz'] = simParams['navdat'].transform(simParams['sys'][1])  # Transform xyzsys.
    simParams['grdxyz'] = simParams['navdat'].toground(simParams['topo'],simParams['sys'][1])  # xyzsys.
...
def grids(confDict, data, simParams, i):
    ...
    Args:
        ...
        i(int): Iterator from runSim.
    """
    unitVec = simParams['navxyz'].getdir(i)  # Approximate direction of travel, unit vector.
    ground_point = simParams['grdxyz'][i]  # Point on ground at nadir.
    simParams['grd_vec'] = Vec(ground_point, simParams['navxyz'][i])  # Vector from nadir to xcvr.
    ...
    ct_dist = int(confDict['facet_params']['ct_dist'])
    at_dist = int(confDict['facet_params']['at_dist'])
    rgrid = makeGrid(simParams['navxyz'][i], unitVec, rvec, at_step, at_dist, ct_step, ct_dist)
    lgrid = makeGrid(simParams['navxyz'][i], unitVec, lvec, at_step, at_dist, ct_step, ct_dist)
...
def c_sim(confDict, data, xcvr, side):
    ...
    c_flist = (c_double * 3 * 3 * len(flist))(*(tuple(tuple(j) for j in i) for i in flist))
    ...
    data[side + 'Twtt'] = list(slib.c_twtt(c_flist, c_xcvr, c_twtt , c_c, c_flen))
...
def averages(confDict, simParams, data, i):
    ...
    if confDict['outputs']['tmap'] == 'True':
        ...
        for j in range(0, len(twtt), step):
            ...
            rdata[k][4], rdata[k][5], rdata[k][6] = fcenAvg(flist[j:j+step])  # Facet center average.
            k += 1
        data['tmap'][i] = rdata
...
def checkSides(confDict, simParams, data, box, index, side):
    ...
    if box > -traceLength and box < traceLength:
        data[side + 'cgram'][box][i] = data[side + 'cgram'][box][i] + data[side + 'Power'][j]
        ...
        if confDict['outputs']['echomap'] == 'True' or confDict['outputs']['echomap_adjusted'] == 'True':
            ...
            if side == 'r':
                data['emap'][simParams['num_ct']-1 + data[side + 'Facet'][j].idx, i] = data['emap'][simParams['num_ct']-1 + data[side + 'Facet'][j].idx, i] + data[side + 'Power'][j]
                data['bins'][simParams['num_ct']-1 + data[side + 'Facet'][j].idx, i] = min(data['bins'][simParams['num_ct']-1 + data[side + 'Facet'][j].idx, i], box)
        ...
        if confDict['outputs']['echomap_ref'] == 'True':
            ...
            simParams['emap_ref'][yAxis,xAxis] = simParams['emap_ref'][yAxis,xAxis] + np.log(data[side + 'Power'][j])
    else:
       data['badf'] += 1
...
def printInfo(simParams, i):
    ...
    sat =  "\033[92m\033[1mSpacecraft elevation = {:.3f}m".format(simParams['grd_vec'].vlen())
...
def combined(confDict, simParams, data, adj):
    ...
    if confDict['outputs']['red'] == 'True':
        ...
        cimg = Image.fromarray(cstack)
        cimg = cimg.convert('RGB')
        cimg.save(confDict['paths']['out_path'] + 'combined' + adj + '_red.png')
...
def binary(confDict, data):
    ...
    cgram.astype('float32').tofile(confDict['paths']['out_path'] + 'combined.img')
...
def csv(confDict, simParams, data):
    ...
    if confDict['outputs']['fret'] == 'True':
        fret_loc = data['fret_loc'].transform(simParams['sys'][0])  # Transform llesys.
        fret_loc.save(confDict['paths']['out_path'] + 'fret.csv', data['minbin'])

    # Produce nadir.csv product.
    if confDict['outputs']['nadir'] == 'True':
        nad_loc = simParams['nad_loc'].transform(simParams['sys'][0])  # Transform llesys.
        nad_loc.save(confDict['paths']['out_path'] + 'nadir.csv', simParams['nadbin'])
...
def GetNav_fritz(navfile):
    ...
    for i in range(len(navdat)):
        ...
        if newr > 400000:
            raise SystemExit("\033[91m\033[1m" + str(i,newr,aer_nadir[i].z, navdat[i].z))
